...
// numerator / denominator as a non-negative rate string at `scale` decimals,
// rounded half away from zero. Used to derive a bank's implied FX rate from the
// local credit and its recorded foreign source amount (e.g. 71875.00 / 25000.00).
export function divideToRate(numerator: string, denominator: string, scale = 6): string {
  const n = parse(numerator);
  const d = parse(denominator);
  if (d.mantissa === 0n) return "0";
  const num = (n.mantissa < 0n ? -n.mantissa : n.mantissa) * pow10(scale + d.scale);
  const den = (d.mantissa < 0n ? -d.mantissa : d.mantissa) * pow10(n.scale);
  const quotient = num / den;
  const remainder = num % den;
  const rounded = remainder * 2n >= den ? quotient + 1n : quotient;
  return format(rounded, scale);
}
